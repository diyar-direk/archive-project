...
const AddReport = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.type)
      ...
      setLoading(true);
      try {
        const data = await axios.post(`${baseURL}/reports`, form, {
        ...
        if (documents.image.length > 0) {
          imagesDoc.append("parentModel", "Report");
        ...
        if (documents.video.length > 0) {
          videosDoc.append("parentModel", "Report");
        ...
        if (documents.audio.length > 0) {
          audioDoc.append("parentModel", "Report");
        ...
        if (uploadedFiles.list.length > 0) {
          documentDoc.append("parentModel", "Report");
  ...
  return (
    <>
      {responseOverlay && (
        <SendData data={"report"} response={response.current} />
      )}
      {loading && <Loading />}
      <h1 className="title">{language?.reports?.add_report}</h1>
        ...
        <div className="form">
          ...
          <div className="flex wrap">
            <InputWithLabel
              ...
              required
              onChange={handleForm}
              placeholder={language?.reports?.report_subject_placeholder}
              id="subject"
              rows={6}
