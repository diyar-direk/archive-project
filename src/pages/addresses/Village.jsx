...
const Village = () => {
  ...
  const getData = useCallback(async () => {
    ...
    try {
      const { data } = await axios.get(`${baseURL}/Villages`, {
        headers: { Authorization: `Bearer ${token}` },
        params,
      });

      dataLength.current =
        data[search ? "numberOfActiveResults" : "numberOfActiveVillages"];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (update) {
        ...
        const data = await axios.post(`${baseURL}/Villages`, form, {
          headers: { Authorization: "Bearer " + token },
        });
        if (data.status === 201) {
          responseFun(true);
  ...
  return (
    <>
      {responseOverlay && (
        <SendData
          data={language?.header?.village}
          response={response.current}
        />
      )}
      {formLoading && <Loading />}
      <h1 className="title">{language?.header?.village}</h1>
      <div className="flex align-start gap-20 wrap">
        {context.userDetails.isAdmin && (
          <form onSubmit={handleSubmit} className="addresses">
            ...
            <InputWithLabel
              label={language?.village?.village_name}
              ref={ref}
              required
